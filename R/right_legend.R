#' right_legend
#'
#' @return A ggplot object with a graphical guide for the scale on the right.
#' @export
#'
#' @examples
#' data <- data.frame(x=factor(c('A','A','A','B','B','C','C')),y=c(10,20,30,40,50,60,70))
#' ggplot2::ggplot(data) + ggplot2::geom_violin(ggplot2::aes(x=x, y=y, fill=x)) + right_legend()

right_legend <- function(){
  #pas d'arguments necessaires
  ggplot2::theme(
    legend.position="right", legend.title=ggplot2::element_text(size=15, face="bold", hjust=0.5),
    #legende a droite et parametrage de la police du titre de legende.
